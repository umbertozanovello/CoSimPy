...
class RF_Coil():
    ...
    def powerBudget(self, p_inc, voxVols=None, elCond=None, printReport=False):
        
        powBudg = {}
        tot_p_inc = np.sum(np.abs(p_inc))
        powBudg["P_inc_tot"] = tot_p_inc
        
        pows = self.s_matrix.powerBudget(p_inc)
        ...
        if self.em_field is None or self.em_field.e_field is None:
            ...
            if len(pows) == 1: # No available data for external circuitries losses
                powBudg["P_refl"] = tot_p_inc - pows[0]
                powBudg["P_other"] = pows[0]
            elif len(pows) == 2:
                powBudg["P_refl"] = tot_p_inc - pows[0]
                powBudg["P_circ_loss"] = pows[0] - pows[1]
                powBudg["P_other"] = tot_p_inc - powBudg["P_refl"] - powBudg["P_circ_loss"]
            ...
            if len(pows) == 1: # No available data for external circuitries losses
                powBudg["P_refl"] = tot_p_inc - pows[0][idxs]
                powBudg["P_dep"] = depPow
                powBudg["P_other"] = pows[0][idxs] - depPow
            elif len(pows) == 2:
                powBudg["P_refl"] = tot_p_inc - pows[0][idxs]
                powBudg["P_dep"] = depPow
                powBudg["P_circ_loss"] = pows[0] - pows[1]
                powBudg["P_other"] = tot_p_inc - powBudg["P_refl"] - powBudg["P_circ_loss"] - powBudg["P_dep"] 
        ...
        if printReport:
            ...
            for i, freq in enumerate(frequencies):
                ...
                for key in powBudg:
                    if key != "P_inc_tot":
                        print("%s: %.2e W\t(%.2f %%)\n"%(key, powBudg[key][i], powBudg[key][i]/tot_p_inc*100))
        return powBudg