...
class S_Matrix():
    ...
    def __resSMatrix(self, other):
        ...
        #Management of singular matrices: Part A
        
        #Look for singular matrices
        mask_sing = np.isclose(np.linalg.det(S_0), 0)
        
        if mask_sing.any():
            flag = False #It becomes True if some singular matrices are not manageble
            #Mask of manageble singular matrices: Array of shape equal to S_0.shape[0],S_0.shape[1]. rc_zero_mask[i,j] is True if S_0[i,:,j] == S_0[i,j,:] == 0
            rc_zero_mask = np.logical_and((S_0==0).all(axis=1),(S_0==0).all(axis=2))
            #Indeces of S_0 first dimension containing singular matrices which can be managed
            inv_S_0_ixds = np.where((rc_zero_mask).any(axis=1))[0]
            
            if len(mask_sing) != len(inv_S_0_ixds):
                flag = True
                print("__resSMatrix WARNING: The matrix is singular at least at one frequency value. NaN is returned at those frequencies")
        ...
        #Management of singular matrices: Part B
        
        if mask_sing.any():
            S_0 = np.copy(self.__S)
            for idx in inv_S_0_ixds:
                #Indeces of the invertible sub-matrix
                rc_idxs = np.where(np.logical_not(rc_zero_mask[idx]))[0]
                sub_S_0 = S_0[idx][rc_idxs][:,rc_idxs]
                
                if np.isclose(np.linalg.det(sub_S_0), 0):
                    if not flag:
                        print("__resSMatrix WARNING: The matrix is singular at least at one frequency value. NaN is returned at those frequencies")
                else:
