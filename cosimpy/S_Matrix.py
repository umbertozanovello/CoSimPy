...
class S_Matrix():
    ...
    def powerBudget(self, p_inc):
        ...
        # vp_1 = np.expand_dims(vp_1,axis=0)
        # vp_1 = vp_1.repeat(self.__n_f, axis=0) #vp_1.shape = (self.__n_f, self.__nPorts)
        ...
        if self.__S0 is None:
            print("No previous connections to external circuitries or connection data have not been saved. The power budget is computed only at the supply ports")
            return p_acc_1,
        ...
        p_inc2 = p_inc2 @ p_inc #p_inc2.shape = [self.__n_f, self.__S0.__nPorts]
        
        vp_2 = np.sqrt(self.__S0.__z0 * np.abs(p_inc2)) * np.exp(1j*np.angle(p_inc2))
