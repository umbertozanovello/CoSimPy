...
if __name__ == "__main__":
    # Scst=S_Matrix.importTouchstone("SP1.s2p")
    # Ssl=S_Matrix.importTouchstone("SPsl.s2p", fmt='dB_Deg', freqUnit='Hz')
    # #       -----
    # #      |
    # #S_c --
    # #      |
    ...
    # S_c[S_c==0] = 1-(1/3.)
    # S_c = [S_c]
    # S_c = np.repeat(S_c,1001,0)
    # S_c_M = S_Matrix(S_c,Scst.frequencies)
    
    # S_op = np.ones((1001,1,1))
    # S_op = S_Matrix(S_op,Scst.frequencies)
    # S_cc = -1*np.ones((1001,1,1))
    # S_cc = S_Matrix(S_cc,Scst.frequencies)
    # S_M = np.zeros((1001,1,1))
    # S_M = S_Matrix(S_M,Scst.frequencies)
    
    # #Hybrid Coupler
    # S_hc = -1/np.sqrt(2)*np.array([[[0,0,1j,1],[0,0, 1,1j],[1j,1,0,0],[1,1j,0,0]]])
    # S_hc = np.repeat(S_hc,Ssl.n_f,0)
    # S_hc = S_Matrix(S_hc,Ssl.frequencies)
    
    frequencies = np.linspace(50e6,250e6,10)
    S50 = S_Matrix.sMatrixRLseries(50,0,frequencies)
    S0 = S_Matrix.sMatrixRLseries(0,0,frequencies)
    ST = S_Matrix.sMatrixTnetwork(S50,S50,S50)
    # ST.S[5,:,1] = 0
    # ST.S[5,1,:] = 0
    # ST.S[-1] = [[0.25+0.j, 0.25+0.j],[0.25+0.j, 0.25+0.j]]
    other = np.zeros((10,4,4))
    other[:,0,2]=1
    other[:,2,0]=1
    # other[:,2,2]=0.25
    other[:,1,1]=0.25
    ...
    Other = S_Matrix(other,frequencies)
